...
namespace ASP.net_8_Entity_Framework_RESTful_API
{
    ...
    public class productsController : ControllerBase
    {
        ...
        public async Task<ActionResult<Response<Product>>> productsNonEF([FromQuery, Required] int value)
        {
            ...
            sqlCommand = new SqlCommand("select TOP " + value.ToString() + " p.ProductId, p.CategoryId, p.ProductName, p.UnitPrice, p.Discontinued from products p", objADONet.connectionSQLServer);
        ...
        public async Task<ActionResult<string>> productsSortedNonEF([FromQuery, Required] string sortField, [FromQuery, Required] string sortCriteria)
        {
            ...
            sqlCommand = new SqlCommand("select p.CategoryId, p.ProductId, p.ProductName, p.UnitPrice, p.Discontinued from products p order by p." + sortField + " " + sortCriteria, objADONet.connectionSQLServer);
            dsData = sqlCommand.ExecuteReader();
